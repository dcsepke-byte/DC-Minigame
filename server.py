...
class Room:
    ...
    # ---- Spieler-Verwaltung ----
    def add_player(self, client, name, pid=None):
        # Reconnect?
        if pid and pid in self.players:
            ...
            client.send({"type": "joined", "playerId": pid, "name": p["name"],
                         "color": p["color"], "code": self.code, "state": self.state})
            self.send_lobby()
            return
        ...
        client.send({"type": "joined", "playerId": new_pid, "name": name,
                     "color": color, "code": self.code, "state": self.state})
        self.send_lobby()
    ...
    # ---- Spielstart ----
    def start_game(self, rounds, order_mode, games):
...
def handle_message(client, msg):
    ...
    if t == "player:join":
        ...
        if not room:
            ...
            return
        room.add_player(client, msg.get("name"), msg.get("playerId"))
        return
    ...
    if t == "host:start" and client.role == "host":
        room.start_game(msg.get("rounds", 5), msg.get("order", "random"), msg.get("games", []))
