...
(async () => {
  ...
  // Screenshot vor der Bewegung
  await page.screenshot({ path: '/opt/data/pa_move_before.png' });
  // Bewege p1 von Feld 0 zu Feld 5 (entlang der Strasse)
  await page.evaluate(() => {
    window.Board2D.animatePawnMove('p1', 0, 5);
  });
  await page.waitForTimeout(1200); // waehrend der Animation
  await page.screenshot({ path: '/opt/data/pa_move_mid.png' });
  await page.waitForTimeout(2000); // nach der Animation
  await page.screenshot({ path: '/opt/data/pa_move_after.png' });
